...
<\body>
  <screens|<\hidden>
    ...
    In the subsequent M step, the distribution q(Z) is held fixed and the
    lower bound L(q,\<theta\>) is maximized with respect to \<theta\> to give
    some new value \<theta\>new. This will cause the lower bound L to
    increase (unless it is already at a maximum), which will necessarily
    cause the corresponding log likelihood function to increase. Because the
    distribution q is determined using the old parameter values rather than
    the new values and is held fixed during the M step, it will not equal the
    new posterior distribution p(Z\|X,\<theta\>new), and hence there will be
    a nonzero KL divergence. The increase in the log likelihood function is
    therefore greater than the increase in the lower bound, as shown in
    Figure 9.13.
  </hidden>|<\hidden>
    <\padded-center>
      <\small-figure|<image|image/fig_9_13_em_m.png|.45par|||>>
        Illustration of the M step of the EM algorithm. The distribution
        <math|q(Z)> is held fixed and the lower bound <math|L(q,\<theta\>)>
        is maximized with respect to the parameter vector <math|\<theta\>> to
        give a revised value \<theta\>new. Because the KL divergence is
        nonnegative, this causes the log likelihood <math|ln p(X\|\<theta\>)>
        to increase by at least as much as the lower bound does.
      </small-figure>
    </padded-center>
  </hidden>|<\hidden>
