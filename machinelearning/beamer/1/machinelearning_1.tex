...
\section{课程简介}
\label{sec:orgdcbd5e1}
\begin{frame}[label={sec:org26761b5}]{机器学习}
\begin{block}{定义}
对于某类任务 \(T\) 和性能度量 \(P\) 如果一个计算机程序在T上以P衡量的性能随着经验E而自我完善，那么我们称这个计算机程序在从经验E学习。
\end{block}
\end{frame}
\begin{frame}[label={sec:orgeb45874}]{课程主要内容}
\begin{itemize}
\item 概念学习
\item 决策树学习
\item 人工神经网络
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org6ea11f1}]{与其它课程关系}
\begin{itemize}
\item 概率与数理统计
\item 人工智能
\item 模式识别
...
\end{frame}
\section{机器学习示例}
\label{sec:org6464b0f}
\begin{frame}[label={sec:org1fca072}]{机器学习示例}
\begin{itemize}
\item 无人驾驶汽车
\item 手写识别
\item 人脸识别
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgbea627c}]{Typical Datamining Task}
\begin{center}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb8956aa},fragile]{Datamining Result}
 \begin{itemize}
\item Data:
\end{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org5ebb056},fragile]{Credit Risk Analysis}
 Data:
...
\end{verbatim}
\end{frame}
\begin{frame}[label={sec:orgde81738}]{Customer purchase behavior:}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/customer-outcomes.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org5813c9a}]{Customer retention:}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/bank-customer.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org51f3820}]{Process optimization:}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/process-outcomes.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org01d070b}]{ALVINN [Pomerleau] drives 70 mph on highways}
...
\end{frame}
\begin{frame}[label={sec:orgfc4dde6}]{Software that Customizes to User}
\center
\begin{center}
...
\end{frame}
\section{学习问题的标准描述}
\label{sec:org785f2f4}
\begin{frame}[label={sec:org078ae45}]{定义}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org8495daa}]{西洋跳棋学习问题：}
\begin{itemize}
\item 任务T：下西洋跳棋
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orged71c50}]{}
\begin{center}
...
\end{frame}
\begin{frame}[label={sec:orgdb764b8}]{手写识别学习问题}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb30e5f2}]{}
\begin{center}
\includegraphics[width=0.8\textwidth]{./image/mnist.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgedb632c}]{机器人驾驶学习问题}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgf8c2e90}]{}
\begin{center}
\includegraphics[width=0.7\textwidth]{./image/tesla-autopilot-full-self-driving-fsd-1400x933.jpeg}
\end{center}
\end{frame}
\section{设计一个学习系统}
\label{sec:orgc713285}


\begin{frame}[label={sec:org98c81c5}]{西洋跳棋学习问题：}
\begin{itemize}
\item 任务T：下西洋跳棋
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org3469549}]{选取训练经验的类型}
\begin{itemize}
\item 直接或间接
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org6ed6102}]{选择目标函数}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org2406b8a}]{定义目标函数（ \(V\) ）}
\begin{itemize}
\item 例：
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org0352658}]{选择目标函数表示}
\begin{itemize}
\item 一组规则
\item 神经网络
\item 棋盘状态的多项式函数
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgd59bb55}]{目标函数表示}
\begin{eqnarray*}
&&\sum_{i=0}^6 w_i f_i \\
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgd140fa4}]{估计训练值}
\begin{itemize}
\item \(V(b)\): 真实目标函数
\item \(\hat{V}(b)\) : 学习到的函数
\item \(V_{train}(b)\): 训练值
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orga10c8ef}]{权值调整}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org2217a94}]{最终设计}
\center
\begin{center}
\includegraphics[height=0.7\textheight]{./image/intro-final-design.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgb49d2b0}]{最终设计}
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org435ef22}]{设计过程}
\begin{center}
\includegraphics[height=0.7\textheight]{./image/intro-f1.png}
\end{center}
\end{frame}
\section{机器学习的问题}
\label{sec:org9b538a7}
...
\end{itemize}
\section{机器学习相关资源}
\label{sec:org12e3064}

\begin{frame}[label={sec:org06745b1}]{课程}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org29b3268}]{资料}
\begin{itemize}
...
\end{frame}
\section{工具}
\label{sec:org9bdc728}
\begin{frame}[label={sec:orgab07a8a}]{C/C++}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgbd866d6}]{Lua}
\begin{itemize}
\item \url{http://torch.ch}
\item \url{https://github.com/torchnet/}
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org6e2b6a2}]{Python}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org3cab0b6}]{Java}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org454fa70}]{科学计算}
\begin{itemize}
\item Rstudio(R)
\item Matlab/Octave
\item Scilab
