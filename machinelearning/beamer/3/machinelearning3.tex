...
\section{决策树介绍}
\label{sec:orgaee501b}
\begin{frame}[label={sec:org79e47ab}]{决策树表示法}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org6b12ecc}]{决策树学习的适用问题}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org73e6128}]{决策树学习的适用问题}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org42c0bba}]{Decision Tree for \(PlayTennis\)}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/dt-f1.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org6c4d7ea},fragile]{A Tree to Predict C-Section Risk}
 \begin{itemize}
...
\end{frame}
\section{基本的决策树学习算法}
\label{sec:org6bcb43a}
\begin{frame}[label={sec:orgd2cce7d}]{ID3(Examples，Target\_Attribute，Attributes)}
\begin{itemize}
\item Examples: 训练样例集。
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org54282a7}]{ID3(Examples，Target\_Attribute，Attributes)}
\begin{itemize}
\item 创建树的Root结点
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc04f602}]{ID3(Examples，Target\_Attribute，Attributes)}
\begin{itemize}
\item 否则:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc7ed656}]{最佳分类属性}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/dt-s1.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org7ee1f36}]{熵（entropy）}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgfe93618}]{熵（entropy）}
\center
\begin{center}
\includegraphics[width=0.5\textwidth]{./image/dt-fig-entropy-new.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgbc470d0}]{信息增益度量}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orga2425a3}]{信息增益度量}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/dt-s1.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:orga12c10e}]{例}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org1969bf9}]{Selecting the Next Attribute}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/dt-inf.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org5948c2f}]{Selecting the Next Attribute}
\center
\begin{center}
\includegraphics[width=0.6\textwidth]{./image/dt-t.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:orga37ad0e}]{Hypothesis Space Search by ID3}
\center
\begin{center}
...
\label{sec:org4918f0b}
\begin{frame}[label={sec:org9382bf8}]{ID3算法的搜索空间}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org171c921}]{ID3算法的搜索策略}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgd41b073}]{ID3算法的搜索策略}
\begin{itemize}
...
\end{frame}
\section{ID3算法归纳偏置}
\label{sec:org1acc801}

\begin{frame}[label={sec:org7baad4d}]{BFS-ID3(Breadth First Search)}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgce72d99}]{ID3归纳偏置：}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org2b8e387}]{ID3算法和候选消除算法(Version Space Candidate Elimination,VSCE)的归纳偏置}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org59d88a1}]{限定偏置和优选偏置}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgd6b5558}]{奥坎姆剃刀 (Occam's Razor)}
优先选择拟合数据的最简单假设。
...
\end{frame}
\section{决策树学习的常见问题}
\label{sec:orgaa5d059}
\begin{frame}[label={sec:org48a57c5}]{过拟合（Overfitting）数据}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org5285398}]{过拟合（Overfitting）数据}
考虑假设 \(h\) 错误率
\begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org9c882e3}]{Overfitting in Decision Tree Learning}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/dt-train-val.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org42abed5}]{避免决策树学习中的过度拟合}
\begin{itemize}
\item 及早停止增长树法，在ID3算法完美分类训练数据之前停止增长树；
\item 后修剪法（post-prune），即允许树过度拟合数据，然后对这个树后修剪。
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgb68cadf}]{确定最终正确树大小的准则}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org30aa553}]{错误率降低修剪}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orga667e86}]{错误率降低修剪的效果}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/dt-prune.png}
\end{center}
\end{frame}
\begin{frame}[label={sec:org5c12235}]{规则后修剪（rule post-pruning）}
规则后修剪包括下面的步骤：
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgcd3773a}]{决策树转化为等价的规则集合}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org36a08c1}]{决策树转化成规则集的好处：}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org623ca20}]{连续值属性}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orga11b047}]{属性选择的其它度量标准}
...
\end{frame}
\begin{frame}[label={sec:org14ae99f}]{缺少属性值的训练样例(Unknown Attribute Values)}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org88ca31b}]{处理代价不同的属性(Attributes with Costs)}
Consider
