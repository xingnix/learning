...
\section{简介}
\label{sec:org411e52f}

\begin{frame}[label={sec:orgdfc2404}]{贝叶斯学习方法的特性：}
\begin{itemize}
...
\begin{frame}[label={sec:orgd34956e}]{贝叶斯公式}
...
\begin{frame}[label={sec:org22b0637}]{最大后验 (Maximum a posteriori,MAP)}
\[ P(h|D) = \frac{P(D|h) P(h)}{P(D)} \]
...
\begin{frame}[label={sec:org330441e}]{极大似然（ Maximum likelihood，ML）}
假定 \(P(h_{i})=P(h_{j})\) 则可进一步简化，选取极大似然假设：

$$h_{ML}=\arg \max_{h \in H}P(D|h)$$
\end{frame}

\begin{frame}[label={sec:org2020b5a}]{基本公式}
\begin{itemize}
...
\section{贝叶斯学习}
\label{sec:orgae79e18}
\begin{frame}[label={sec:org5b24356}]{概念}
\begin{itemize}
...
\begin{frame}[label={sec:orgb6ea4dd}]{Brute-Force MAP学习算法}
\begin{itemize}
...
\begin{frame}[label={sec:org3d94a89}]{Brute-Force MAP学习算法（续）}
...
\begin{frame}[label={sec:org35cecce}]{Brute-Force MAP学习算法（续）}
可得：
...
\begin{frame}[label={sec:orgd8860d6}]{贝叶斯法则}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/bayes-vs.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgfdce572}]{贝叶斯学习}
\begin{center}
\includegraphics[width=0.7\textwidth]{./image/vs-map-equivalent.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:orga815cb9}]{MAP假设和一致学习器}
\begin{itemize}
...
\section{极大似然}
\label{sec:org8009b0d}
\begin{frame}[label={sec:org61bf813}]{学习实值函数}
\begin{center}
\includegraphics[width=0.7\textwidth]{./image/bayes-linear.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org02ddb8d}]{考虑实值函数 \(f\)}
\begin{itemize}
...
\begin{frame}[label={sec:org8a8e977}]{推导：}
\begin{eqnarray}
h_{ML} &= &\arg \max_{h \in H} p(D|h) \nonumber \\
...
\begin{frame}[label={sec:orgf522311}]{用自然对数替换，得：}
\begin{eqnarray}
...
\begin{frame}[label={sec:orge6c0f34}]{用于预测概率的极大似然假设}
考虑从数据中预测概率
\begin{itemize}
...
\section{最小描述长度准则}
\label{sec:org645f46a}
\begin{frame}[label={sec:orgca5bb52}]{奥坎坶剃刀 （Occam's razor）}
\begin{itemize}
...
\begin{frame}[label={sec:orgbab7ce1}]{示例：}
\begin{itemize}
\item \(H\) = 决策树
\item \(D\) = 训练数据
...
\begin{frame}[label={sec:org81ac341}]{由信息论可得：}
\begin{quote}
...
\section{贝叶斯最优分类器}
\label{sec:org6b3841a}
\begin{frame}[label={sec:org2424251}]{新实例的最大可能分类}
\begin{itemize}
...
\begin{frame}[label={sec:orgec20818}]{贝叶斯最优分类器}
$$\arg \max_{v_{j} \in V} \sum_{h_{i} \in H} P(v_{j}|h_{i}) P(h_{i}|D)$$
\end{frame}

\begin{frame}[label={sec:orgeba55e7}]{示例:}
\begin{eqnarray}
...
\section{GIBBS 算法}
\label{sec:orgf6823b1}
\begin{frame}[label={sec:orgb8b9789}]{GIBBS 算法}
\begin{itemize}
...
\begin{frame}[label={sec:orga04f246}]{Gibbs算法:}
\begin{itemize}
...
\begin{frame}[label={sec:org78e4ce6}]{概念学习问题分析：}
\begin{itemize}
...
\section{朴素贝叶斯分类器}
\label{sec:orgb6775b0}
\begin{frame}[label={sec:org1a1bc6e}]{朴素贝叶斯分类器（ Naive Bayes Classifier ）}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgbc1983f}]{描述}
\begin{itemize}
...
\begin{frame}[label={sec:org01cff4b}]{算法}
\begin{itemize}
...
\begin{frame}[label={sec:org91b8380}]{示例}
\begin{itemize}
...
\begin{frame}[label={sec:orgfd10573}]{Naive Bayes: Subtleties}
\begin{itemize}
\item 通常不满足独立性假定
$$P(a_{1}, a_{2} \ldots a_{n}|v_{j}) = \prod_{i} P(a_{i} | v_{j})$$
...
\begin{frame}[label={sec:orgce8f55c}]{当目标值为 \(v_j\) 的所有训练实例都没有属性值 \(a_i\)? 时}
...
\begin{frame}[label={sec:org53db20c}]{学习分类文本}
\begin{itemize}
\item 学习将文本按兴趣分类
\item 学习将网页按主题分类
\end{itemize}
...
\begin{frame}[label={sec:org17ddc0e}]{将文档表示为单词向量}
\begin{itemize}
...
\begin{frame}[label={sec:org3e7ea36}]{朴素贝叶斯条件独立假定}
...
\begin{frame}[label={sec:orgc2a7b71}]{算法流程}
...
\begin{frame}[label={sec:orge76d8c9}]{算法流程(续)}
\begin{itemize}
...
\begin{frame}[label={sec:orgede1d4a}]{算法流程(续)}
...
\begin{frame}[label={sec:orgba62da6}]{Twenty NewsGroups}
\small
...
\begin{frame}[label={sec:orgf78be6a},fragile]{Article from rec.sport.hockey}
 \small
\begin{verbatim}
...
\begin{frame}[label={sec:org745dec0}]{Learning Curve for 20 Newsgroups}
\begin{center}
...
\section{贝叶斯信念网}
\label{sec:orga554912}

\begin{frame}[label={sec:org00e0448}]{贝叶斯信念网(Bayesian Belief Networks)}
\begin{itemize}
...
\begin{frame}[label={sec:orgd63ebf8}]{条件独立}
\begin{quote}
...
\begin{frame}[label={sec:org3a2dffe}]{示例:}
...
\begin{frame}[label={sec:org462e091}]{网络}
\begin{center}
\includegraphics[width=.9\linewidth]{./image/bayesnet.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgc27c00b}]{说明}
\begin{itemize}
...
\begin{frame}[label={sec:orgc1a8f0e}]{表示联合概率 ：}
\begin{itemize}
...
\begin{frame}[label={sec:orgab2eb46}]{贝叶斯网络推理}
\begin{itemize}
...
\begin{frame}[label={sec:org436ea41}]{学习贝叶斯网络}
\begin{itemize}
...
\begin{frame}[label={sec:orgc83030e}]{梯度上升算法}
\begin{itemize}
...
\begin{frame}[label={sec:orge319499}]{梯度上升算法(续)}
\begin{itemize}
...
\begin{frame}[label={sec:orgad1bc8b}]{More on Learning Bayes Nets}
可使用 EM 算法
\begin{itemize}
...
\section{EM}
\label{sec:org6d93179}

\begin{frame}[label={sec:orgb5ae440}]{Expectation Maximization (EM)}
\begin{itemize}
\item 观测到部分数据
\item 实例的部分属性未知
\item 无监督聚类
...
\begin{frame}[label={sec:orgbbd382e}]{Generating Data from Mixture of \(k\) Gaussians}
\begin{center}
...
\begin{frame}[label={sec:orgbcb6376}]{EM for Estimating \(k\) Means}
已知:
\begin{itemize}
...
\begin{frame}[label={sec:org8175d67}]{EM Algorithm:}
...
\begin{frame}[label={sec:org6b8e6f1}]{EM Algorithm:}
\begin{itemize}
\item M step:
\begin{itemize}
...
\begin{frame}[label={sec:org937c4e4}]{EM Algorithm}
\begin{itemize}
...
\begin{frame}[label={sec:orga6091f2}]{General  EM Problem}
已知:
\begin{itemize}
...
\begin{frame}[label={sec:orgdc6fc7d}]{General  EM Problem}
