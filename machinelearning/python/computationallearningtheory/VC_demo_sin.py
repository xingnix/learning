...
prim=[2,3]+list(filter(lambda r:min(map(lambda x: r%x,range(2,int(np.sqrt(r)+1))))!=0,range(4,N)))
print ("compute axis...")
x=[[] for i in range(n)]
for i in range(0,2**n):
	...
	for j in range(0,len(b)):
		if(b[-j-1]=='1'):
			x[j].append(int(prim[i]))
index=[reduce(lambda m,n: m*n,i) for i in x]

print ("random test...")
for i in range(5):
    data=(np.random.randint(0,2,n)-0.5)*2
    period=int(prim[int(sum(np.double(data==1)*2**np.array(range(len(data)))))])
    print ("result  ", i  , "  :")
