...
\end{frame}
\begin{frame}
\frametitle{零度根轨迹的八大法则(续)}
\label{sec-3-2-3}

\begin{itemize}
\item <2->根轨迹的起始角与终止角
      \begin{eqnarray*}
      \theta_{p_i} & = & \left[\sum_{j=1}^m\angle(p_i-z_j)-\sum_{\substack{j=1 \\ j\neq i}}^n\angle(p_i-p_j)\right] \\
      \phi_{z_j} & = & \left[\sum_{\substack{i=1 \\ i\neq j}}^m\angle(z_j-z_i)-\sum_{i=1}^n\angle(z_j-p_i)\right] 
      \end{eqnarray*}
\item <3->根轨迹与虚轴交点
\begin{itemize}
...
\item 利用Routh判据计算
\end{itemize}
\item <4->根之和: $n-m\geq 2$ 时, 闭环极点之和等于开环极点之和
\end{itemize}
\end{frame}
\begin{frame}
\frametitle{零度根轨迹示例1:}
