...
\begin{enumerate}
\item 例:\hfill{}\textsc{B\_block}
\label{sec:org0b3bded}
...
\subsubsection{Nichols图}
\label{sec:org8bb30cd}
\begin{itemize}
...
\end{enumerate}
\section{典型环节频率特性}
\label{sec:org65193f1}
\subsection{比例,积分微分环节}
\label{sec:orgd51d683}
\subsubsection{比例环节}
...
\subsubsection{比例环节(续)  \(G(j\omega) = K,K=1\)}
\label{sec:orgdf3115d}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:orgc4c0c4a}
\begin{tikzpicture}[scale=0.5]
%g=1;
\begin{axis}[
grid=both,
...
\end{axis}
\end{tikzpicture}
\item Bode图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:org1194535}
\begin{tikzpicture}[scale=0.5]
%g=1
\begin{semilogxaxis}[
grid=both,
...
L(\omega)&=& -20\lg\omega
\end{eqnarray*}
\subsubsection{积分环节(续)\(G(j\omega)  = \frac{1}{j\omega}\)}
\label{sec:orge2ca22a}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:org19666d4}
\begin{tikzpicture}[scale=0.5]
%g=1/s
\begin{axis}[
grid=both,
...
L(\omega)&=& 20\lg\omega
\end{eqnarray*}
\subsubsection{微分环节(续) \(G(j\omega)  = j\omega\)}
\label{sec:org4d2a31d}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:orgc44594a}
\begin{tikzpicture}[scale=0.5]
%g=s
\begin{axis}[
%axis x line=middle,axis y line= middle, 
...
\label{sec:org2368c6b}
\subsubsection{惯性环节}
\label{sec:orgf79fac0}
\begin{eqnarray*}
G(s) & = & \frac{1}{Ts+1}\\
...
\end{eqnarray*}
\subsubsection{惯性环节(续) \(G(j\omega) = \frac{1}{j\omega T+1},T=1\)}
\label{sec:orgec454fc}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{B\_block:BMCOL}
\label{sec:org0334fde}
\begin{tikzpicture}[scale=0.5]
%g=1/(s+1)
\begin{axis}[
%axis x line=middle,axis y line= middle, 
...
L_a(\omega)&=& \begin{cases} 0 & \omega<\frac{1}{T} \\  20\lg\omega T & \omega>\frac{1}{T}\end{cases}
\end{eqnarray*}
\subsubsection{一阶微分环节(续) \(G(j\omega) = j\omega T+1,T=1\)}
\label{sec:orge6b117d}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:orga9b8ba7}
\begin{tikzpicture}[scale=0.5]
%g=(s+1)
\begin{axis}[
%axis x line=middle,axis y line= middle, 
...
\end{itemize}
\end{itemize}
\subsubsection{二阶振荡环节(续):\(G(j\omega) = \frac{1}{1+2j\xi\omega T-\omega^2 T^2},T=1\)}
\label{sec:orgff36652}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:org25d8833}
\begin{tikzpicture}[scale=0.5]
%g=1/(T^2 s+2\xi Ts+1)
\begin{axis}[
%axis x line=middle,axis y line= middle, 
...
\label{sec:org67eca79}
\begin{eqnarray*}
G(s) & = & (Ts)^2+2\xi Ts+1 \\
G(j\omega) & =& 1+2j\xi\omega T-\omega^2 T^2\\
\end{eqnarray*}
\subsubsection{二阶微分环节(续)\(G(j\omega) = 1+2j\xi\omega T-\omega^2 T^2,T=1\)}
\label{sec:orgd88b7b2}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:org592cddb}
\begin{tikzpicture}[scale=0.5]
%g=(T^2 s+2\xi Ts+1)
\begin{axis}[
%axis x line=middle,axis y line= middle, 
...
\subsection{非最小相位环节}
\label{sec:org29dd7d9}
\subsubsection{延迟环节}
\label{sec:org1c8e45e}
\begin{eqnarray*}
G(s) & = & e^{-\tau s}\\
...
\subsubsection{延迟环节(续)\(G(j\omega)  = e^{-j\omega\tau},\tau=1\)}
\label{sec:org71058b1}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:org7a6feba}
\begin{tikzpicture}[scale=0.5]
%g=e^{-\tau s};
\begin{axis}[
grid=both,
...
\subsubsection{非最小相位惯性环节}
\label{sec:org788807d}
最小相位系统:在右半平面无零极点
\begin{eqnarray*}
G(s) & = & \frac{1}{Ts-1}\\
G(j\omega) & =& \frac{1}{j\omega T-1}\\
...
\subsubsection{非最小相位惯性环节(续)\(G(j\omega) = \frac{1}{j\omega T-1},T=1\)}
\label{sec:orge8d8cda}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:orgc9716f7}
\begin{tikzpicture}[scale=0.5]
%g=1/(s-1)
\begin{axis}[
%axis x line=middle,axis y line= middle, 
...
\end{enumerate}
\section{系统开环频率特性}
\label{sec:org35359a2}
\subsection{开环系统Nyquist图}
\label{sec:org7e55228}
\subsubsection{开环系统Nyquist图 \(G_o(s) =\frac{K\prod_{j=1}^m(\tau_j s+1)}{s^{\nu}\prod_{i=1}^{n-\nu}(T_i s+1)}\)}
\label{sec:org9d323ec}
\begin{itemize}
\item 当  \(\nu=0\)  时,为零型系统:
\begin{eqnarray*}
...
\end{eqnarray*}
\end{itemize}
\subsubsection{开环系统Nyquist图(续) \(G_o(s) =\frac{K\prod_{j=1}^m(\tau_j s+1)}{s^{\nu}\prod_{i=1}^{n-\nu}(T_i s+1)}\)}
\label{sec:orgb5c985c}
\begin{itemize}
\item 当  \(\nu=1\)  时 ,为I型系统:
\begin{eqnarray*}
...
\subsubsection{开环系统Nyquist图,例1}
\label{sec:orga5691fc}
\begin{enumerate}
\item \(G_o(s)=\frac{10}{s^{\nu}(0.1s+1)}\)\hfill{}\textsc{BMCOL:B\_block}
\label{sec:org9887e47}
\begin{tikzpicture}[scale=0.5]
% $G_o(s)=\frac{10}{s^{\nu}(0.1s+1)}$ 
\begin{axis}[
%axis x line=middle,axis y line= middle, 
...
\item \(G_o(s)=\frac{10}{(0.1s+1)^{n}}\)\hfill{}\textsc{BMCOL:B\_block}
\label{sec:orgc62ec80}
\begin{tikzpicture}[scale=0.5]
...
\subsubsection{开环系统Nyquist图,例2:\(G(s)=\frac{10}{s(s+1)(2s+1)(4s+1)}\)}
\label{sec:org9433b3c}
\begin{itemize}
...
\end{eqnarray*}
\end{itemize}
\subsubsection{开环系统Nyquist图,例2(续),\(G(s)=\frac{10}{s(s+1)(2s+1)(4s+1)}\)}
\label{sec:org14b2b8b}
概略Nyquist图:
...
\end{itemize}
\subsubsection{开环系统Nyquist图,例2(续),\(G(s)=\frac{10}{s(s+1)(2s+1)(4s+1)}\)}
\label{sec:orgc39e6d6}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:orgd04dbc9}
\begin{tikzpicture}[scale=0.5]
% $G(s)=\frac{10}{s(s+1)(2s+1)(4s+1)}$ 
\begin{axis}[
%axis x line=middle,axis y line= middle, 
...
\item 局部放大:\hfill{}\textsc{BMCOL:B\_block}
\label{sec:orgc39518e}
\begin{tikzpicture}[scale=0.5]
% $G(s)=\frac{10}{s(s+1)(2s+1)(4s+1)}$ 
\begin{axis}[
%axis x line=middle,axis y line= middle, 
...
\label{sec:org34fabad}
\subsubsection{开环系统Bode图}
\label{sec:orga6189d2}
\begin{eqnarray*}
...
\subsubsection{开环系统Bode图,例1: \(G_o(s)=\frac{10(s+3)}{s(s+2)(s^2+s+2)}\)}
\label{sec:org9b53f5d}
绘制Bode图:
\begin{enumerate}
...
\item <6->找转折点依次做直线即可
\end{enumerate}
\subsubsection{开环系统Bode图,例1(续): \(G_o(s)=\frac{10(s+3)}{s(s+2)(s^2+s+2)}\)}
\label{sec:org77c0de3}
Bode图	
...
\section{频域稳定性判据}
\label{sec:org3b978a8}
\subsection{Nyquist稳定性判据}
\label{sec:orgba1cad3}
\subsubsection{辐角原理}
\label{sec:org68c07e6}
\begin{itemize}
...
\end{itemize}
\subsubsection{辐角原理(续):}
\label{sec:orgcad49ce}
...
\item 将 \(\Gamma\) 分为四段:\hfill{}\textsc{BMCOL:B\_block}
\label{sec:org3b4a2c8}
\begin{itemize}
...
\subsubsection{穿越次数}
\label{sec:org6177ff3}
\begin{enumerate}
\item Nyquist图\hfill{}\textsc{BMCOL:B\_block}
\label{sec:orgc0b76f3}
\begin{tikzpicture}[scale=0.5]
\begin{axis}[
axis x line=middle,axis y line= middle, 
ylabel=$j$ ,xlabel=$   $ ,
...
\item 穿越次数\hfill{}\textsc{BMCOL:B\_block}
\label{sec:orgc18d999}
\begin{itemize}
...
\subsubsection{例: \(G_o(s)=\frac{10}{s(s+1)}\)}
\label{sec:org6a02ce6}
...
\label{sec:org9d2b416}
\subsubsection{Bode稳定性判据}
\label{sec:org3318aef}
\begin{enumerate}
\item Bode图\hfill{}\textsc{BMCOL:B\_ignoreheading}
\label{sec:org5e433f1}
...
\label{sec:org2c085a2}
\subsubsection{等 \(\alpha\) 曲线}
\label{sec:org28e6b1e}
\begin{align*}
G(j\omega) &= Ae^{j\phi} \\
...
\end{align*}
\subsubsection{等 \(M\) 曲线}
\label{sec:org7cfd5b7}
\begin{align*}
\frac{Ae^{j\phi}}{Me^{j\alpha}}&=1+Ae^{j\phi} \\
\frac{A}{M}&=|1+Ae^{j\phi}|\\
...
\end{align*}
\subsubsection{Nichols Chart}
\label{sec:orgefb86c3}
\begin{center}
\includegraphics[width=.9\linewidth]{image/nichols_chart.eps}
\end{center}

\subsection{指标转换}
\label{sec:orgd3ebc61}
\subsubsection{系统闭环和开环和频域指标的关系}
\label{sec:org50907e7}
\begin{align*}
...
\subsubsection{２阶系统频域指标}
\label{sec:org118dae0}
\begin{align*}
...
\end{align*}
\subsubsection{2阶系统频域指标( \(M_r,\omega_r\) )}
\label{sec:org19b6522}
\begin{eqnarray*}
